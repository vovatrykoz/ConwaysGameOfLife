namespace Conway.App.File

open Conway.App.Controls
open Conway.Encoding
open Raylib_cs
open System
open System.IO

type BinaryCanvasFileLoader(gameDecoder: IConwayByteDecoder) =

    member val Decoder = gameDecoder with get

    member _.DecodeCameraInfo(cameraInfoBytes: ReadOnlySpan<byte>) =
        let posX = BitConverter.ToSingle(cameraInfoBytes.Slice(0, 4))
        let posY = BitConverter.ToSingle(cameraInfoBytes.Slice(4, 4))
        let zoomFactor = BitConverter.ToSingle(cameraInfoBytes.Slice(8, 4))

        Camera(LanguagePrimitives.Float32WithMeasure posX, LanguagePrimitives.Float32WithMeasure posY, zoomFactor)

    member this.Decode(byteEncoding: ReadOnlySpan<byte>) =
        let camera = this.DecodeCameraInfo(byteEncoding.Slice(0, 12))
        let game = this.Decoder.Decode(byteEncoding.Slice(12).ToArray())

        game, camera

    interface ICanvasFileLoader with
        member this.Load(path: string) : CanvasWrapper =
            try
                let encoding = File.ReadAllBytes path
                let game, camera = this.Decode encoding

                {
                    Game = game
                    Camera = camera
                    OptionalMessage = None
                }
            with
            | :? ArgumentNullException as argNullEx ->
                Raylib.TraceLog(
                    TraceLogLevel.Error,
                    $"Could not open the file. {argNullEx.ParamName} was null or empty"
                )

                reraise ()
            | :? ArgumentException ->
                if String.IsNullOrEmpty path then
                    Raylib.TraceLog(TraceLogLevel.Error, $"{nameof path} was empty")
                    reraise ()
                else if String.IsNullOrWhiteSpace path then
                    Raylib.TraceLog(TraceLogLevel.Error, $"{nameof path} consisted of whitespaces only")
                    reraise ()
                else
                    let invalidCharSet = Path.GetInvalidPathChars() |> Set.ofArray

                    let invalidChars =
                        path |> Seq.filter (fun c -> Set.contains c invalidCharSet) |> Seq.toArray

                    Raylib.TraceLog(
                        TraceLogLevel.Error,
                        $"{nameof path} contained the following invalid characters: {invalidChars}"
                    )

                    reraise ()
            | :? PathTooLongException ->
                Raylib.TraceLog(TraceLogLevel.Error, $"The provided {nameof path} was too long: {path}")
                reraise ()
            | :? DirectoryNotFoundException ->
                Raylib.TraceLog(TraceLogLevel.Error, $"The provided {nameof path} was not found: {path}")
                reraise ()
            | :? IOException ->
                Raylib.TraceLog(
                    TraceLogLevel.Error,
                    $"An I/O error occurred while opening the file at {path}. Does the file exist?"
                )

                reraise ()
            | :? Security.SecurityException ->
                Raylib.TraceLog(TraceLogLevel.Error, $"The caller does not have enough permissions to open the file")
                reraise ()
            | :? UnauthorizedAccessException ->
                Raylib.TraceLog(
                    TraceLogLevel.Error,
                    $"Access to the file at {path} was denied. This could be due to one of the following:
                    (1) the caller does not have the required permission
                    (2) {path} is a directory
                    (3) operation is not supported on the current platform"
                )

                reraise ()
            | :? NotSupportedException ->
                Raylib.TraceLog(TraceLogLevel.Error, $"The provided {nameof path} has an invalid format: {path}")

                reraise ()
            | _ ->
                Raylib.TraceLog(TraceLogLevel.Error, $"Something went wrong")
                reraise ()
